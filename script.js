...
let currentAudiometryResults = null; // Module-scoped variable instead of window
...
function populateSelects() {
    const defaultTexts = {
        'saved-audiometries': 'Seleccionar para cargar...',
        'select-audiometry-compare-1': 'Seleccionar Audiometría Basal',
        'select-audiometry-compare-2': 'Seleccionar Audiometría de Seguimiento'
    };
    const selects = [
    ...
    selects.forEach(select => {
        ...
        const defaultText = defaultTexts[select.id] || 'Seleccionar...';
...
function calculateAndDisplay() {
    ...
    ['od', 'oi'].forEach(ear => {
        ...
        FREQUENCIES.forEach(freq => {
            vaValues[freq] = parseInt(document.getElementById(`va-${ear}-${freq}`).value) || 0;
            voValues[freq] = parseInt(document.getElementById(`vo-${ear}-${freq}`).value) || 0;
        });
    ...
    currentAudiometryResults = { ...results, binauralDeficiency };
...
// --- Smart Copy Helper (HTML + Plain Text) ---
async function copyToClipboard(html, text) {
    try {
        ...
        // Fallback to text only if something goes wrong
        try {
...
// Helper para generar fila de tabla (Reutilizable)
...
// --- Generadores de Texto Plano (Para Copiar) ---
function generatePlainTextTable(data) {
    const header = "Hz\t" + FREQUENCIES.join("\t");
    const rowVA = "VA\t" + FREQUENCIES.map(f => !isNaN(data.va[f]) ? data.va[f] : '-').join("\t");
    const rowVO = "VO\t" + FREQUENCIES.map(f => !isNaN(data.vo[f]) ? data.vo[f] : '-').join("\t");
    return `${header}\n${rowVA}\n${rowVO}`;
}

function generatePlainTextBasicReport() {
    const { od, oi } = currentAudiometryResults;

    // INFORME BÁSICO: Solo Tablas y Diagnóstico (Estricto)
    return `RESULTADOS DE AUDIOMETRÍA

Oído Derecho (OD):
${generatePlainTextTable(od)}
Diagnóstico OD: ${od.diagnosis}

Oído Izquierdo (OI):
${generatePlainTextTable(oi)}
Diagnóstico OI: ${oi.diagnosis}
`;
}

function generatePlainTextDetailedReport() {
    const { od, oi, binauralDeficiency } = currentAudiometryResults;
    ...
    // INFORME DETALLADO: Todo el contenido completo
    return `INFORME DETALLADO DE AUDIOMETRÍA
Fecha: ${document.getElementById('audiometry-date').value || 'N/D'}
Nº Historia: ${document.getElementById('history-number').value || 'N/D'}

--- 1. UMBRALES AUDITIVOS (dB HL) ---
Oído Derecho (OD):
${generatePlainTextTable(od)}
Diagnóstico OD: ${od.diagnosis}
...
--- 2. CÁLCULO DE DEFICIENCIA Y DISCAPACIDAD (Orden DSA/934/2023 y RD 888/2022) ---
...
Deficiencia Binaural: ${binauralDeficiency.binauralDeficiency.toFixed(2)}%
...
function generateBasicReport() {
    ...
    // INFORME BÁSICO HTML: Solo Tablas y Diagnóstico (Estricto)
    return `
        <h3>Umbrales Auditivos y Diagnóstico</h3>
...
function generateDetailedReport() {
    ...
    // generateFreqRow is now defined globally for this file above
...
function copyTable() {
    ...
    // 2. HTML Version (Actual Table) - Adding inline styles for compatibility
    ...
    copyToClipboard(tableHTML, tableText)
        .then(success => {
            if (success) showToast('Tabla copiada (Formato Inteligente).', 'success');
            else showToast('Error al copiar tabla.', 'error');
        });
}

function copyBasicReport() {
    const text = generatePlainTextBasicReport();
    const html = document.getElementById('basic-report-content').innerHTML;

    copyToClipboard(html, text)
        .then(success => {
            if (success) showToast('Informe básico copiado.', 'success');
            else showToast('Error al copiar informe.', 'error');
        });
}

function copyDetailedReport() {
    const text = generatePlainTextDetailedReport();
    const html = document.getElementById('detailed-report-content').innerHTML;

    copyToClipboard(html, text)
        .then(success => {
            if (success) showToast('Informe detallado copiado.', 'success');
            else showToast('Error al copiar informe.', 'error');
        });
}

function copyComparisonReport() {
    // Smart Copy for Comparison too
    const container = document.getElementById('comparison-report-content');
    const text = container.innerText;
    const html = container.innerHTML;

    copyToClipboard(html, text)
        .then(success => {
            if (success) showToast('Informe comparativo copiado.', 'success');
            else showToast('Error al copiar informe.', 'error');
        });
}
...
// Initialize print modal event listeners once
